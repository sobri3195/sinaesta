# Usulan Penyempurnaan Sistem Sinaesta (Paling Berdampak)

Dokumen ini merangkum prioritas perbaikan lintas sistem dengan menyeimbangkan kebutuhan pengguna, alur UX, performa, keamanan, dan kemudahan implementasi.
...
2. **Akurasi user journey kritikal (auth → onboarding → exam) belum sepenuhnya terukur end-to-end.**
   Fitur banyak, namun guardrail KPI funnel dan deteksi drop-off belum menjadi bagian inti rilis.
...
## Prioritas 1 — **Stabilisasi Journey Kritis Pengguna (Auth + Onboarding + Exam Session Reliability)**

**Kenapa paling berdampak (bisnis):**
...
**Kenapa feasible (teknis):**
- Arsitektur auth dan API sudah tersedia; fokusnya penyelarasan flow, state handling, dan fallback terstandar.
...
**Kenapa feasible (teknis):**
- Optimasi terarah (code splitting, lazy load, memoization, virtualization, cache strategy) bisa bertahap dan terukur.

## Prioritas 3 — **Security & Operational Readiness (Hardening + Observability + Quality Gates)**
...
3. **Optimasi frontend cepat**
   - Lazy-load modul non-kritis, splitting route, optimasi bundle vendor.
...
5. **Experimentation loop**
   - A/B test untuk onboarding dan exam UX agar peningkatan berbasis data, bukan asumsi.
